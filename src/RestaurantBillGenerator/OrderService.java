...
public class OrderService {
    ...
    public void createOrder(DishService dishService) {
        int dishCode;
        do {
            System.out.println("Lütfen ürün kodu giriniz: (Cikis icin 0 giriniz)");
            dishCode = inp.nextInt(); //code ile ürünü bulma --->12.adim
            if (dishService.findByDishCode(dishCode) != null) {
                ...
                if (findByOrderByDish(dish) != null) {
                    ...
                    order = new Order(dish, number);//orderCode:
                    order.setPrice();
                    this.orderList.add(order);
                }

           //Siparis olusunca altta siparis listesini görüntüle
        ...
        for (Order order:this.orderList){
            if(order.orderCode==code){
                ...
                if(order.numOfDish>num){
                    ...
                    System.out.println("Sipariş iptal edildi: " + order.dish.getName()+ order.numOfDish + " adet");
                }else {
